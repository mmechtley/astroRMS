...
from scipy.ndimage import median_filter, uniform_filter, binary_dilation, \
    binary_erosion, minimum_filter, maximum_filter
import datetime
...
_log_info = {
    ...
    'weightScale': 'Factor to scale supplied weight map by to create inverse ' +
                   'variance map',
...
def object_mask(input_data, sky_bin=4, sky_size=15, smooth_size=3,
    ...
    if ((sky_size > 0 and sky_size % 2 != 1) or
       (smooth_size > 0 and smooth_size % 2 != 1)):
        raise ValueError(
            'sky_size and smooth_size must both be odd integers, or 0')
    ...
    sky, rms = pixel_stats(workingData, clip_sig=3, n_clip=num_clips)
...
def pixel_stats(pixels, clip_sig=3, n_clip=10, min_pix=50):
    clip_iter = 0
    mode, sig = 0, 1
...
def autocorrelation_phot(autocorr_image, aper_radius=5,
    ...
    skyMask = (sqDist > bg_annulus_radii[0] ** 2) & (
        sqDist < bg_annulus_radii[1] ** 2)
    ...
    skyWts = (np.where(skyMask, 1, 0)
              + np.where(skyFixMask,
                         skyFixArea / autocorr_image[skyFixMask].size, 0))
    ...
    fluxWts = (np.where(fluxMask, 1, 0)
               + np.where(fluxFixMask,
                          fluxFixArea / autocorr_image[fluxFixMask].size, 0))
...
def binned_median_filter(image_data, bin_size=4, filt_size=25):
    ...
    if bin_size > 1:
        binned = np.add.reduceat(
            np.add.reduceat(image_data, np.arange(0, imgShape[0], bin_size),
                            axis=0),
...
def calc_RMS(image_data, weight_data=None, sky_bin=4, sky_size=25,
    ...
    if mask_sigma > 0:
        bpMask |= object_mask(workingData, sky_size=0,
                              smooth_size=mask_smooth_size,
                              threshold=mask_sigma,
                              grow_size=mask_grow_size).astype(bool)

    fracFlagged = bpMask.sum() / image_data.size
    ...
    if use_rms_filt:
        ...
        bpMask = binary_dilation(bpMask, structure=struct_elem)
        bpMask |= rmsFiltered <= 0.0  # Reject weights < 0
        ## Find histogram peak of the masked, rms-filtered pixels
        medRMS, rmsRMS = pixel_stats(rmsFiltered[~bpMask], clip_sig=3,
                                     n_clip=10)
        measuredRMS = medRMS
